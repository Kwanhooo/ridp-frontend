  ...
  useEffect(() => {
    const fetch_data = async () => {
      try {
        ...
        const initialBridge = bridge_options[0];
        ...
        const initialTime = time_options[0] ? formatTime(time_options[0]) : "";
        const initialMetric = metric_options[0] ?? "";
        ...
        // 确保只调用一次 handleQuery
        if (!cache.current && initialTime && initialMetric) {
          cache.current = true; // 标记已经调用过 handleQuery
          await handleQuery(initialBridge, initialTime, initialMetric);
        }
      } catch (error) {
  ...
  const handleQuery = async (
    ...
    if (checkParameters(bridge, time, metric)) {
      try {
        ...
        setAnomalyFilling(
          anomalyFillingResponse.result ??
            anomalyFillingResponse.data ??
            []
        );
        setMissingFilling(
          missingFillingResponse.result ?? missingFillingResponse.data ?? []
        );
      } catch (error) {
        console.error("Error fetching data:", error);
      } finally {
    ...
    const fetchData = async () => {
      if (checkParameters(selectedBridge, selectedTime, selectedMetric)) {
        try {
          ...
          setAnomalyFilling(
            fillingResult.result ?? fillingResult.data ?? []
          );
        } catch {
          console.log("error");
        } finally {
